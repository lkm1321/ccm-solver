function xcombdot =  Construct_LorenzSync(t,xcomb,Wd,WdO, rholistC, rholistO, ustar,B,t_lead,x_lead)
%-------------------------------------------------------------------------%
...
xstar = interp1(t_lead,x_lead,t);                   % Time varying input
xstar = xstar'; 
n = 3;                                              % Number of state
...
if (length(rholist)>1)
    const = rholist(1) + rholist(5)*xcurr(1)^2 + rholist(7)*xcurr(2)^2 ...
            + rholist(9)*xcurr(2)*xcurr(3) + rholist(10)*xcurr(3)^2 ...
            + rholist(4)*xcurr(3) + rholist(6)*xcurr(1)*xcurr(2);
    lin = 2*(rholist(5)*xcurr(1)*xdiff(1) + rholist(7)*xcurr(2)*xdiff(2)...
            + rholist(10)*xcurr(3)*xdiff(3)) + rholist(9)*(xcurr(2)*xdiff(3)...
            + xcurr(3)*xdiff(2)) + rholist(4)*xdiff(3) + rholist(6)*(xcurr(1)*xdiff(2)...
            + xcurr(2)*xdiff(1));
    quad = rholist(5)*xdiff(1)^2 + rholist(7)*xdiff(2)^2 ...
            + rholist(9)*xdiff(2)*xdiff(3) + rholist(10)*xdiff(3)^2 ...
            + rholist(6)*xdiff(1)*xdiff(2);
...
if (length(rholistO)>1)
    ...
    const = rholist(1) + rholist(5)*xbar(1)^2 + rholist(7)*xbar(2)^2 ...
            + rholist(9)*xbar(2)*xbar(3) + rholist(10)*xbar(3)^2 ...
            + rholist(4)*xhat(3) + rholist(6)*xhat(1)*xhat(2);
    lin = 2*(rholist(5)*xbar(1)*xdiff(1) + rholist(7)*xbar(2)*xdiff(2) ...
            + rholist(10)*xbar(3)*xdiff(3)) + rholist(9)*(xbar(2)*xdiff(3)...
            + xbar(3)*xdiff(2)) + rholist(4)*xdiff(3) ...
            + rholist(6)*(xhat(1)*xdiff(2) + xhat(2)*xdiff(1));
    quad = rholist(5)*xdiff(1)^2 + rholist(7)*xdiff(2)^2 ...
            + rholist(9)*xdiff(2)*xdiff(3) + rholist(10)*xdiff(3)^2 ...
            + rholist(6)*xdiff(1)*xdiff(2);    
...
% State equations
xdot = [sigma*(xcurr(2) - xcurr(1)) ;
        r*xcurr(1) - xcurr(3) ;
        - bb* xcurr(3)] + B*u;
xhatdot = [sigma*(xhat(2) - xhat(1)) ;
        r*xcurr(1) - xhat(3) ;
        - bb* xhat(3)] + B*u - uc;
