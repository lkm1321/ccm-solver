...
G = [x2 - 1; -x1 + 1];          % Null space of the Jacobian of desired manifold is just dz/dx * f = 0.
B = [B, G]; 
...
[sol, q, Q, ~] = solvesos(constraints, [], options, coeffList);
...
controller = YalmipPolynomial([x1, x2], 2, 1); 
...
LMI2 = (A*W + W*A' - B*K*W - W*K'*B' - rho2.polyRep*(f*f') + 2*freeLambda*W );
